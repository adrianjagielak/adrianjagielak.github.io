acsbVKbootstrap = function(){

    if( typeof jQuery === 'undefined' || parseInt(jQuery.fn.jquery.split('.')[0]) < 2 && parseInt(jQuery.fn.jquery.split('.')[1]) < 7 ) {
    ...
    EJS.eve.listen(document, 'DOMNodeInserted', {}, function(e){

        if( e.target.nodeType === 1 && e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA' ) {
            acsbVKbindFocus(jQuery(this));
        }
...
acsbVKbindFocus = function($elements){

    if( ! $elements ) {
        $elements = jQuery('textarea, input');
    }
    ...
    $elements.on('focus', function(){
        ...
        if( typeof type === 'undefined' || EJS.arr.inArray(['text', 'number', 'email', 'search', 'url', 'password', 'tel'], type) ) {

            if( ! $input.hasClass('js-acsb-triggered') ) {
                acsbVKtrigger($input);
                $input.addClass('js-acsb-triggered').focus();
            }
        }
...
acsbVKloadBaseScript = function(){

    if( window.acsbVkInited ) {
        return true;
    }

    (function(){

        var $keyboard = jQuery.keyboard = function(el, opts){
            var o, base = this;
            ...
            base.init = function(){
                ...
                if( opts && opts.position ) {
                    position      = jQuery.extend({}, opts.position);
                    opts.position = null;
                }
                base.options = o = jQuery.extend(true, {}, $keyboard.defaultOptions, opts);
                if( position ) {
                    o.position    = position;
                ...
                base.wheel               = jQuery.isFunction(jQuery.fn.mousewheel);
            ...
            base.setCurrent = function(){
                var kbcss    = $keyboard.css,
                    ...
                    $current = jQuery('.'+kbcss.isCurrent),
                    kb       = $current.data('keyboard');
                // close keyboard, if not self
                if( ! jQuery.isEmptyObject(kb) && kb.el !== base.el ) {
                    kb.close(kb.options.autoAccept ? 'true' : false);
                }
                $current.removeClass(kbcss.isCurrent);
                // ui-keyboard-has-focus is applied in case multiple keyboards have
                // alwaysOpen = true and are stacked
                jQuery('.'+kbcss.hasFocus).removeClass(kbcss.hasFocus);
            ...
            base.reveal = function(redraw){
                ...
                jQuery('.'+kbcss.keyboard).not('.'+kbcss.alwaysOpen).each(function(){
                    var kb = jQuery(this).data('keyboard');
                    if( ! jQuery.isEmptyObject(kb) ) {
                ...
                if( o.openOn ) {
                    base.$el.unbind(jQuery.trim((o.openOn+' ').split(/\s+/).join(base.namespace+' ')));
                }

                // build keyboard if it doesn't exist; or attach keyboard if it was removed, but not cleared
                if( ! base.$keyboard || base.$keyboard &&
                    (! base.$keyboard.length || jQuery.contains(base.el.ownerDocument.body, base.$keyboard[0])) ) {
                    base.startup();
                }
            ...
            base.updateLanguage = function(){
                ...
                lang          = (jQuery.isArray(lang) ? lang[0] : lang);
                base.language = lang;
                lang          = lang.split('-')[0];

                // set keyboard language
                o.display      = jQuery.extend(true, {},
                    kblang.en.display,
                    kblang[lang] && kblang[lang].display || {},
                    base.settings.display
                );
                o.combos       = jQuery.extend(true, {},
                    kblang.en.combos,
            ...
            base.startup = function(){
                ...
                if( o.reposition && jQuery.ui && jQuery.ui.position && o.appendTo == 'body' ) {
                    jQuery(window).bind('resize'+base.namespace, function(){
                        base.reposition();
                    });
                }
            ...
            base.reposition = function(){
                base.position = jQuery.isEmptyObject(o.position) ? false : o.position;
                // position after keyboard is visible (required for UI position utility)
                // and appropriately sized
                if( jQuery.ui && jQuery.ui.position && base.position ) {
                    base.position.of        =
            ...
            base.makePreview = function(){
                if( o.usePreview ) {
                    ...
                    attrs       = base.$preview[0].attributes;
            ...
            base.setScroll = function(){
                ...
                if( ! base.isContentEditable && base.last.virtual ) {
                    ...
                    var scrollWidth, clientWidth, adjustment, direction,
                        ...
                        val        = base.last.val.substring(0, Math.max(base.last.start, base.last.end));
                    ...
                    if( isTextarea ) {
                        ...
                        base.$previewCopy.val(val.replace(/\s/g, '\xa0'));
            ...
            base.bindKeyboard = function(){
                    ...
                    .bind('keypress'+namespace, function(e){
                        ...
                        if( o.restrictInput ) {
                            ...
                            if( (e.which === keyCodes.backSpace || e.which === 0) &&
                                jQuery.inArray(e.which, base.alwaysAllowed) ) {
                                return;
                            }
                            // quick key check
                            if( jQuery.inArray(str, layout.acceptedKeys) === -1 ) {
                                e.preventDefault();
                ...
                base.$keyboard.bind('mousedown click touchstart '.split(' ').join(base.namespace+' '), function(e){
                    ...
                    if( ! base.isCurrent() ) {
                        base.reveal();
                        jQuery(base.el.ownerDocument).trigger('checkkeyboard'+base.namespace);
                    }
                    base.setFocus();
                });
                ...
                base.$allKeys = base.$keyboard.find('button.'+kbcss.keyButton)
                    ...
                    .bind('mouseenter mouseleave touchstart '.split(' ').join(base.namespace+' '), function(e){
                        ...
                        if( o.useWheel && base.wheel ) {
                            ...
                            last.wheelIndex  = jQuery.inArray($this.attr('data-value'), txt);
                        }
                    ...
                    .bind(o.keyBinding.split(' ').join(base.namespace+' ')+base.namespace+' '+
                        ...
                        } else if( action in $keyboard.keyaction && jQuery.isFunction($keyboard.keyaction[action]) ) {
                        ...
                        if( typeof action !== 'undefined' && action !== null ) {
                            last.key = jQuery(this).hasClass(kbcss.keyAction) ? action : last.key;
            ...
            base.checkCombos = function(){
                ...
                if( o.useCombos ) {
                    // keep 'a' and 'o' in the regex for ae and oe ligature (ÃƒÂ¦,Ã…â€œ)
                    // thanks to KennyTM: http://stackoverflow.com/q/4275077
                    // original regex /([`\'~\^\"ao])([a-z])/mig moved to jQuery.keyboard.comboRegex
                    if( $keyboard.msie ) {
                ...
                if( o.restrictInput && val !== '' ) {
                    ...
                    if( ! r ) {
                        t2 = jQuery.map(layout.acceptedKeys, function(v){
            ...
            base.switchInput = function(goToNext, isAccepted){
                if( jQuery.isFunction(o.switchInput) ) {
                    ...
                    var kb,
                        stopped = false,
                        all     = jQuery('button, input, select, textarea, a, [contenteditable]')
                            .filter(':visible')
                            .not(':disabled'),
            ...
            base.close = function(accepted){
                if( base.isOpen && base.$keyboard.length ) {
                    ...
                    if( accepted && jQuery.isFunction(o.validate) && ! o.validate(base, val, true) ) {
            ...
            base.escClose = function(e, $el){
                ...
                var shouldStayOpen = false,
                    $target        = $el || jQuery(e.target);
                EJS.each(base.escCloseCallback, function(i, callback){
            ...
            base.processKeys = function(name){
                ...
                data.title = jQuery.trim(data.title).replace(/_/g, ' ');
                return data;
            };
            ...
            base.buildKeyboard = function(name, internal){
                ...
                if( jQuery.isEmptyObject(o.display) ) {
                    // set keyboard language
                    base.updateLanguage();
                }
                ...
                index = jQuery.inArray('{space}', acceptedKeys);
                if( index > -1 ) {
                    acceptedKeys[index] = ' ';
                }
                ...
                EJS.each($layout, function(set, keySet){
                    ...
                    if( set !== '' && ! /^(name|lang|rtl)$/i.test(set) ) {
                        ...
                        for( row = 0; row < keySet.length; row++ ) {
                            ...
                            currentSet = jQuery.trim(keySet[row]).replace(/\{(\.?)[\s+]?:[\s+]?(\.?)\}/g, '{$1:$2}');
                ...
                layout.hasMappedKeys = ! (jQuery.isEmptyObject(layout.mappedKeys));
                ...
                for( key = 0; key < keys.length; key++ ) {
                    ...
                    if( /^\{\S+\}$/.test(keys[key]) ) {
                        ...
                        if( /^sp:((\d+)?([\.|,]\d+)?)(em|px)?$/i.test(action) ) {
                            ...
                            jQuery('<span class="'+kbcss.keyText+'"></span>')
            ...
            base.removeBindings = function(namespace){
                ...
                if( base.el.ownerDocument !== document ) {
                    jQuery(base.el.ownerDocument).unbind(namespace);
                }
                jQuery(window).unbind(namespace);
        ...
        $keyboard.keyaction = {
            ...
            enter     : function(base, el, e){
                ...
                if( tag === 'TEXTAREA' && jQuery(e.target).closest('button').length ) {
        ...
        $keyboard.defaultOptions = {
            ...
            // if user inputs `a the script converts it to Ãƒ , ^o becomes ÃƒÂ´, etc.
            useCombos : true,
            ...
            // 'isClosing' is true
            validate : function(keyboard, val, isClosing){
                return true;
            }
        ...
        $keyboard.checkCaretSupport = function(){
            if( typeof $keyboard.checkCaret !== 'boolean' ) {
                ...
                var $temp = jQuery('<div style="height:0px;width:0px;overflow:hidden;position:fixed;top:0;left:-100px;">'+
        ...
        $keyboard.wrapBRs = function(container){
            var $el = jQuery(container).find('br:not(.'+$keyboard.css.divWrapperCE+')');
            if( $el.length ) {
                EJS.each($el, function(i, el){
                    ...
                    if(
                        ...
                        jQuery(el).addClass($keyboard.css.divWrapperCE).wrap('<div>');
                    }
                });
            }
        };
        ...
        $keyboard.getEditableCaret = function(container){
            ...
            var end, text,
                opts   = (jQuery(container).data('keyboard') || {}).options,
                doc    = container.ownerDocument,
                range  = doc.getSelection().getRangeAt(0),
                result = pathToNode(range.startContainer, range.startOffset),
                start  = result.position;
            if( opts.wrapBRs !== false ) {
        ...
        jQuery.fn.keyboard = function(opts){
            return this.each(function(){
                if( ! jQuery(this).data('keyboard') ) {
                    /*jshint nonew:false */
                    (new jQuery.keyboard(this, opts));
                }
            });
        };

        jQuery.fn.getkeyboard = function(){
            return this.data('keyboard');
        };

        jQuery.fn.caret = function(start, end, noFocus){
            if(
    ...
    jQuery.keyboard.language.he = {
        ...
        display      : {
            'a'      : '\u2714:××™×©×•×¨ (Shift+Enter)', // check mark - same action as accept
            'accept' : '××™×©×•×¨:××™×©×•×¨ (Shift+Enter)',
            'alt'    : 'Alt:×ª×•×•×™× × ×•×¡×¤×™×',
            'b'      : '\u2190:Backspace',    // Left arrow (same as &larr;)
            'bksp'   : 'Bksp:Backspace',
            'c'      : '\u2716:×‘×™×˜×•×œ (Esc)', // big X, close - same action as cancel
            'cancel' : '×‘×™×˜×•×œ:×‘×™×˜×•×œ (Esc)',
            'clear'  : 'C:× ×™×§×•×™',             // clear num pad
            'combo'  : '\u00f6:Toggle Combo Keys',
            'dec'    : '.:× ×§×•×“×” ×¢×©×¨×•× ×™×ª',           // decimal point for num pad (optional), change '.' to ',' for European format
            'e'      : '\u21b5:Enter',        // down, then left arrow - enter symbol
            'enter'  : 'Enter:Enter',
            'lock'   : '\u21ea Lock:Caps Lock', // caps lock
            's'      : '\u21e7:Shift',        // thick hollow up arrow
            'shift'  : 'Shift:Shift',
            'sign'   : '\u00b1:×©×™× ×•×™ ×¡×™×ž×Ÿ',  // +/- sign for num pad
            'space'  : '&nbsp;:×¨×•×•×—',
            't'      : '\u21e5:Tab',          // right arrow to bar (used since this virtual keyboard works with one directional tabs)
            'tab'    : '\u21e5 Tab:Tab'       // \u21b9 is the true tab symbol (left & right arrows)
        },
        // Message added to the key title while hovering, if the mousewheel plugin exists
        wheelMessage : '× ×™×ª×Ÿ ×œ×”×©×ª×ž×© ×‘×’×œ×’×œ×ª ×”×¢×›×‘×¨ ×›×“×™ ×œ×¨××•×ª ×ž×§×©×™× × ×•×¡×¤×™×',
        // language direction
        rtl          : true
    };
    jQuery.keyboard.language.it = {
        language     : 'Italiano (Italian)', // e.g. 'Ð ÑƒÑÑÐºÐ¸Ð¹ (Russian)'
        display      : {
    ...
    jQuery.keyboard.language.ru = {
        ...
        display      : {
            'a'      : '\u2714:Ð¡Ð¾Ñ…Ñ€Ð°Ð½Ð¸Ñ‚ÑŒ (Shift+Enter)', // check mark - same action as accept
            'accept' : 'Ð¡Ð¾Ñ…Ñ€Ð°Ð½Ð¸Ñ‚ÑŒ:Ð¡Ð¾Ñ…Ñ€Ð°Ð½Ð¸Ñ‚ÑŒ (Shift+Enter)',
            'alt'    : 'Ð Ð£Ð¡:Ð ÑƒÑÑÐºÐ°Ñ ÐºÐ»Ð°Ð²Ð¸Ð°Ñ‚ÑƒÑ€Ð°',
            'b'      : '\u2190:Ð£Ð´Ð°Ð»Ð¸Ñ‚ÑŒ ÑÐ¸Ð¼Ð²Ð¾Ð» ÑÐ»ÐµÐ²Ð°',    // Left arrow (same as &larr;)
            'bksp'   : '\u21e6:Ð£Ð´Ð°Ð»Ð¸Ñ‚ÑŒ ÑÐ¸Ð¼Ð²Ð¾Ð» ÑÐ»ÐµÐ²Ð°',
            'c'      : '\u2716:ÐžÑ‚Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ (Esc)', // big X, close - same action as cancel
            'cancel' : 'ÐžÑ‚Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ:ÐžÑ‚Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ (Esc)',
            'clear'  : 'C:ÐžÑ‡Ð¸ÑÑ‚Ð¸Ñ‚ÑŒ',             // clear num pad
            'combo'  : '\u00f6:Toggle Combo Keys',
            'dec'    : ',:Decimal',           // decimal point for num pad (optional), change '.' to ',' for European format
            'e'      : '\u21b5:Ð’Ð²Ð¾Ð´',        // down, then left arrow - enter symbol
            'enter'  : 'Ð’Ð²Ð¾Ð´:ÐŸÐµÑ€ÐµÐ²Ð¾Ð´ ÑÑ‚Ñ€Ð¾ÐºÐ¸',
            'lock'   : '\u21ea Lock:Caps Lock', // caps lock
            's'      : '\u21e7:Ð’ÐµÑ€Ñ…Ð½Ð¸Ð¹ Ñ€ÐµÐ³Ð¸ÑÑ‚Ñ€',        // thick hollow up arrow
            'shift'  : '\u21e7:Ð’ÐµÑ€Ñ…Ð½Ð¸Ð¹ Ñ€ÐµÐ³Ð¸ÑÑ‚Ñ€',
            'sign'   : '\u00b1:Ð¡Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ Ð·Ð½Ð°Ðº',  // +/- sign for num pad
            'space'  : 'ÐŸÑ€Ð¾Ð±ÐµÐ»:',
            't'      : '\u21e5:Tab',          // right arrow to bar (used since this virtual keyboard works with one directional tabs)
            'tab'    : '\u21e5 Tab:Tab'       // \u21b9 is the true tab symbol (left & right arrows)
        },
    ...
    window.acsbVkInited = true;
};

acsbVKtrigger = function(el){

    var $element = jQuery(el);

    if( ! $element.hasClass('ui-keyboard-input') ) {

        var layout   = 'qwerty',
            language = 'en',
            rtl      = false;

        switch( acsb.state.widget.language ) {
        ...
        $element.keyboard({
            ...
            change            : function(e, keyboard, el){
                EJS.eve.trigger(el, 'input')
            },
            accepted          : function(e, keyboard, el){
                EJS.eve.trigger(el, 'change')
            },
            canceled          : function(e, keyboard, el){
                EJS.eve.trigger(el, 'change')
            },
